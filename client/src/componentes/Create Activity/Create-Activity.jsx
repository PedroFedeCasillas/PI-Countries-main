...
function CreateActivity() {
  ...
  function handleSumit(e) {
    ...
    if (
      ...
      setError(
        "El nombre no debe contener caracteres especiales y debe ser mayor a tres"
      );
      setMostrarError(true);
      return;
    }
    const durationRegex = /^([0-1]\d|2[0-3]):([0-5]\d)$/;
    if (!durationRegex.test(state.duration)) {
      setError("Ingrese la duración válida.");
      setMostrarError(true);
      return;
    }
    if (!difficulty) {
          ...
          <form onSubmit={handleSumit}>
            ...
            <div className={styles.contenedor}>
              ...
              <input
                ...
                onChange={(e) => handleChoose(e)}
              />
              <label className={styles.label}>Otoño 🍁</label>
              <input
                className={styles.input}
                type="radio"
                id="Autumn"
                ...
                onChange={(e) => handleChoose(e)}
              />
              <label className={styles.label}>Invierno 🥶</label>
              <input
                className={styles.input}
                type="radio"
                id="Winter"
                ...
                onChange={(e) => handleChoose(e)}
              />
              <label className={styles.label}>Primavera 🌸</label>
              <input
                className={styles.input}
                type="radio"
                id="Spring"
